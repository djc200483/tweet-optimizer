...
app.post('/generate-video', authMiddleware, async (req, res) => {
  try {
    ...
    // Prepare input for Replicate Kling model
    const input = {
      prompt,
      aspect_ratio: aspectRatio,
      duration,
    ...
    console.log('Creating video prediction with input:', JSON.stringify(predictionInput, null, 2));
    const prediction = await replicate.predictions.create(predictionInput);
    console.log('Initial prediction created:', JSON.stringify(prediction, null, 2));
    ...
    let finalPrediction = await replicate.predictions.get(prediction.id);
    while (finalPrediction.status !== 'succeeded' && finalPrediction.status !== 'failed') {
      console.log('Waiting for video prediction...', {
        status: finalPrediction.status,
        id: prediction.id
      });
